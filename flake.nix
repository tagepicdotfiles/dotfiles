{
    ...
    outputs = { self, nixpkgs, home-manager, wallpaper-collection, nur }@inputs: {
        nixosConfigurations.kraken = nixpkgs.lib.nixosSystem {
            system = "x86_64-linux";
            modules = [
                nur.nixosModules.nur
                home-manager.nixosModules.default
                (import ./machines/kraken/configuration.nix inputs)
            ];
        };
        nixosConfigurations.sylveon = nixpkgs.lib.nixosSystem {
            ...
            modules = [
                home-manager.nixosModules.default
                (import ./machines/sylveon/configuration.nix inputs)
            ];
        };
    };
}
